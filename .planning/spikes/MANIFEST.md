...
| 006 | gsd-skill-interaction-matrix | standard | Inventory all GSD skills/agents touching state-bearing MD; classify each as BLOCKS/REDIRECTS/UNAFFECTED under the PreToolUse hook; produce the canonical `/gsd-beads-*` substitute list | PENDING | gsd-ecosystem, hooks, skill-inventory |
| 007 | reader-skill-format-contract | standard | Document the exact regex/parse expectations of every GSD skill/agent that READS state-bearing MD; build a regenerated-MD template that satisfies all of them | PENDING | gsd-ecosystem, regen, format-contract |
| 008 | todowrite-vs-bd-collision | standard | Resolve the conflict between Claude Code's built-in TodoWrite tool (used by GSD skills for in-session tracking) and bd's CLAUDE.md rule "do NOT use TodoWrite"; decide carve-out / hook-intercept / convention | PENDING | gsd-ecosystem, todowrite, claude-code, bd-rules |
| 009 | gsd-new-project-e2e | standard | Run `/gsd-new-project` (the canonical fresh-start flow) end-to-end against our hook on a controlled fixture; verify the onboarding flow doesn't break under the gsd-beads layer | PENDING | gsd-ecosystem, e2e, onboarding |
| 010 | bd-aware-gsd-agents | standard | Probe whether individual upstream GSD agents (gsd-roadmapper, gsd-add-phase, etc.) can be made bd-aware via in-prompt detection without forking GSD; test the boundary of "layer alongside, do not fork" | PENDING | gsd-ecosystem, agents, in-prompt |
| 011 | gsd-spike-wrap-up-integration | standard | Verify `/gsd-spike-wrap-up` (the natural next-step after this session) doesn't accidentally write to state-bearing paths or otherwise conflict with the hook layer | PENDING | gsd-ecosystem, wrap-up, self-test |
