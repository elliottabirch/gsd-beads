#!/usr/bin/env bash
# Test harness for spike 012.
#
# Drives synthetic Claude Code Bash payloads through block-gsd-sdk-mutation.sh
# and asserts the right decisions come out. Writes results/results.json
# and prints a summary table.
set -uo pipefail

cd "$(dirname "$0")"

HOOK="./hooks/block-gsd-sdk-mutation.sh"
...
run_test() {
  ...
  results=$(printf '%s' "$results" | jq \
    --arg name "$name" --arg cmd "$cmd" --arg expect "$expect" \
    --arg got "$got" --arg reason "$reason" --arg status "$status" \
    '. += [{name:$name,cmd:$cmd,expect:$expect,got:$got,reason:$reason,status:$status}]')
